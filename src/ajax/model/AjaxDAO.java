...
public class AjaxDAO implements InterAjaxDAO {
	...
	*/
	...
	// === *** 검색된 회원을 보여주는 메소드 생성하기 *** === // 
		@Override	
		public List<MemberVO> getSearchMembers() throws SQLException {
			...
			try {
				...
				while(rs.next()) {
					...
					MemberVO membervo = new MemberVO(idx, userid, v_name, pwd, email, hp1, hp2, hp3, post1, post2, addr1, addr2,  
					                                 gender, birthday.substring(0, 4), birthday.substring(4, 6), birthday.substring(6), coin, point,  
					                                 registerday, status);
		...
		@Override
		public List<MemberVO> getSearchMembers(String searchname) throws SQLException {
			List<MemberVO> memberList = null; 
			try {
				conn = ds.getConnection();
				// 객체 ds 를 통해 아파치톰캣이 제공하는 DBCP(DB Connection pool)에서 생성된 커넥션을 빌려온다.
	...
	public List<HashMap<String, String>> getImages() throws SQLException {
		...
		try {
			...
			while(rs.next()) {
				...
				hashimg.put("userid", rs.getString("userid"));
				hashimg.put("name", rs.getString("name"));
				hashimg.put("img", rs.getString("img"));
					imgList.add(hashimg);
			}
		} finally {close();}
		return imgList;
	...
	public List<HashMap<String, String>> getAllBooks() throws SQLException {
		...
		try {
			...
			while(rs.next()) {
				...
				hashbook.put("subject", rs.getString("subject"));
				hashbook.put("title", rs.getString("title"));
				hashbook.put("author", rs.getString("author"));
				hashbook.put("registerday", rs.getString("registerday"));
				booklist.add(hashbook);
			}
		} finally {close();}
		return booklist;
	}
	...
	// === *** 테이블 tbl_wordLargeCategory 에서 lgcategorycode 와 codename 을 가져오는(select) 메소드 생성하기 *** // 
		@Override
		public List<HashMap<String, String>> getLgcategorycode() throws SQLException {
			...
			try {
				...
				while(rs.next()) {
					cnt++;
					if(cnt == 1) lgcategorycodeList = new ArrayList<HashMap<String, String>>();
	...
	// === *** 테이블 tbl_wordLargeCategory 에서 lgcategorycode 컬럼의 값이 제일적은 1개만 가져오는 추상 메소드 *** //
		@Override
		public String getMinLgcategorycode() throws SQLException{
			String code = null;
			try {
				conn = ds.getConnection();
				...
				if(bool) code = rs.getString("lgcategorycode");
			} finally{close();}
			return code;
	...
	// === *** 테이블 tbl_wordMiddleCategory 에서 mdcategorycode 와 codename 을 가져오는(select) 메소드 생성하기 *** // 
		@Override
		public List<HashMap<String, String>> getMdcategorycode(String lgcategorycode) throws SQLException {
			...
			try {
				...
				while(rs.next()) {
					cnt++;
					if(cnt == 1) mdcategorycodeList = new ArrayList<HashMap<String, String>>();
	...
	// === *** tbl_wordsearchtest 테이블에 insert 해주는 메소드 생성하기 *** //
		@Override
		public int addWord(HashMap<String, String> map) throws SQLException {
			int n = 0;
			try {
				conn = ds.getConnection();
				// 객체 ds 를 통해 아파치톰캣이 제공하는 DBCP(DB Connection pool)에서 생성된 커넥션을 빌려온다.
	...
	// === *** 검색어 입력시 글자동완성을 위해 tbl_wordsearchtest 테이블의 제목에서 해당글자가 포함된 데이터를 가져오는(select) 메소드 생성하기 *** // 
		@Override	
		public List<String> getSearchedTitle(String searchword) throws SQLException {
			...
			try {
				...
				while(rs.next()) {
					cnt++;
					if(cnt == 1) stringList = new ArrayList<String>();
				    String title = rs.getString("TITLE"); 
	...
	// === *** 검색조건에 맞는 총 게시물갯수 알아오기 *** //  
		@Override
		public int getTotalCountByMdcode(String mdcode) throws SQLException {
			int totalCount = 0;
			try {
				conn = ds.getConnection();
	...
	@Override
		public int getTotalCountBySearchword(String searchword) throws SQLException {
			int totalCount = 0;
			try {
				conn = ds.getConnection();
				String sql = "select count(*) AS CNT "
	...
	// === *** tbl_wordsearchtest 테이블에서 중분류 코드에 해당하는 글내용 또는 글제목에 해당하는 글내용을 조회해주는 추상 메소드 *** //
		@Override
		public List<HashMap<String, String>> getSearchContent(int sizePerPage, int currentShowPageNo, String mdcode, String searchword) throws SQLException {
			...
			try {
			    ...
			    */
				...
				while(rs.next()) {
					cnt++;
					if(cnt == 1) mapList = new ArrayList<HashMap<String, String>>();
