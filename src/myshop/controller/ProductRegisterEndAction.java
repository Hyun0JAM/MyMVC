...
public class ProductRegisterEndAction extends AbstractController {
	...
	public void excute(HttpServletRequest req, HttpServletResponse res) throws Exception {
		...
		// 1. 첨부 파일이 어느 경로에 업로드 할 것인지 그 경로를 설정해야 한다.
		HttpSession session = req.getSession();
		ServletContext svlCtx = session.getServletContext();
		String imagesDir = svlCtx.getRealPath("/images"); 
		System.out.println("=== 첨부되어지는 이미지 파일이 올라갈 절대경로 imagesDir ==> " + imagesDir);
		// === 첨부되어지는 이미지 파일이 올라갈 절대경로 imagesDir ==> C:\myjsp\.metadata\.plugins\org.eclipse.wst.server.core\tmp0\wtpwebapps\MyMVC\images
		MultipartRequest mtreq = null;
		// MultipartRequest mtreq 은 
		// HttpServletRequest req 가 하던일을 그대로 승계받아서 일처리를 해주고
		// 동시에 파일을 받아서 업로드, 다운로드 해주는 기능도 해준다.
		try {
		 ...
		 */
		  mtreq = new MultipartRequest(req, imagesDir, 10*1024*1024, "UTF-8", new DefaultFileRenamePolicy());
			...
			req.setAttribute("msg", "파일업로드 실패 ==> 용량 10MB 초과했으므로 실패함!!");
			req.setAttribute("loc", "productRegister.do");
			super.setRedirect(false);
			super.setViewPage("MyMVC/WEB-INF/msg.jsp");
			return;
		}
		// 2. 첨부 이미지 파일을 올렸으니 그 다음으로는 상품정보(상품명, 정가, 제품설명...)를 
		//    DB의 jsp_product 테이블에 insert 를 해주어야 한다. 
